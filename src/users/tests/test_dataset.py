...
=======
        ...
        obj = Dataset.objects.create(UserId = id, filename=filename,data=dataset)
        obj.save()
        o = Dataset.objects.get(UserId = id,filename=filename)
        
        ## Check for success 
        self.assertEquals(id,o.UserId)
        self.assertEquals(filename,o.filename)
        x = pd.read_json(dataset).to_numpy()
        y = pd.read_json(o.data).to_numpy()
        self.assertEquals(x.all(),y.all())
        ##Check if it fails 
        self.assertNotEquals(o.UserId,8)
        self.assertNotEquals(o.filename,"train.odt")
...
=======
    def test_resolve_data(self):
        ...
        # Success Test cases 
        self.assertEquals(type(filename),str)
        self.assertEquals(type(id),int)
        self.assertEquals(type(dataset),str)

        #Failure Test cases
        self.assertNotEquals(type(filename),dict)
        self.assertNotEquals(type(id),str)
        self.assertNotEquals(type(dataset),dict)
...
=======
        #Success Test cases
        self.assertEquals(list(pd.read_json(dataset).columns),list(pd.read_json(o.data).columns))
        self.assertEquals(pd.read_json(dataset).shape[0],pd.read_json(o.data).shape[0])
        self.assertEquals(pd.read_json(dataset).shape[1],pd.read_json(o.data).shape[1])
        self.assertEquals(nullValues,o.nullValues)


        #Failure Test Cases
        self.assertNotEquals(pd.read_json(dataset).shape[0],0)
        self.assertNotEquals(pd.read_json(dataset).shape[1],0)
        self.assertNotEquals(nullValues,-1)
    ...
    def test_dataset_get_data(self):
        ...
        # check that the data is all stored correctly
        self.assertEquals(x.all(),y.all())
        #Fail when data is incorrect 
        self.assertNotEquals(x.all(),y.any())
...
=======
        ...
        ##Check for failures
        self.assertNotEquals(len(results),-1)
        self.assertNotEquals(round(results[8],2),0)
        self.assertNotEquals(results[10],100)
        self.assertNotEquals(len(results[0]['0']),1)
>>>>>>> fdf01c02abc1ef942f524492c003ca2d3c8e99c7
