...
import pandas as pd
# Create your tests here.
from LogisticRegression.models import LogisticTrainedModel
from LogisticRegression.views import TrainLogisticRegression
...
class NaiveBayesModel(TestCase):
    ...
    def test_correct_model_extracted(self):
        ...
        ## ensure that the dataset is a Logistic Regression one
        self.assertEquals(self.model,"Logistic Regression")
    ...
    def test_creating_file_wih_tag(self):
        ...
        ## check if uploaded file has tag Logistic Regression
    ...
    def test_NaiveBayes_Training(self):
        results = []
        try:
            results = TrainLogisticRegression(pd.read_json(self.dataset),self.id,self.filename)
    ...
    def test_f1_values(self):
        ...
        self.assertAlmostEquals(f1[0]['score'],0.845360824742267)
        self.assertAlmostEquals(f1[1]['score'],0.5871559633027521)
    ...
    def test_auc_scores(self):
        ...
        auc = results[2]
        ##check that each score has a value
        self.assertAlmostEquals(auc[0]['value'],0.6850425838752686)
        self.assertAlmostEquals(auc[1]['value'],0.6850425838752686)
    ...
    def test_upload_to_db(self):
        ...
        result = False
        try:
            LogisticTrainedModel.objects.create(UserId=self.id,filename=self.filename,TrainingAccuracy=ac,f1score=results[1],AUCScore=results[2])
            result = True
        except:
    ...
    def test_UserId(self):
        self.obj = Dataset.objects.create(UserId = self.id, filename=self.filename,data=self.dataset,model = "Logistic Regression")
        self.obj.save()
        FromDb = Dataset.objects.get(UserId =self.id,filename=self.filename,model =  "Logistic Regression")
    ...
    def test_nullValues(self):
        self.obj = Dataset.objects.create(UserId = self.id, filename=self.filename,data=self.dataset,nullValues=self.nullValues,model = "Logistic Regression")
        self.obj.save()
