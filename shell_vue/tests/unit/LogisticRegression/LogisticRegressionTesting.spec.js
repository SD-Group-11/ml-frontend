...
import LogisticRegressionTesting from '@/views/mlModels/LogisticRegression/LogisticRegressionTesting.vue'
...
function factory(){
  return shallowMount(LogisticRegressionTesting,{
    global: { 
      plugins: [store]
    },
    mocks: {
...
describe('NaiveBayesTraining.vue', () => {
  ...
  it('check wether the correct api call is made to get the user datasets info',async()=>{
    ...
    await flushPromises()
    expect(mockAxiosPost.mock.calls[0][0]).toEqual('/LogisticRegression/getDatasetsInfo',{
      'UserId':1,
    })
  })
